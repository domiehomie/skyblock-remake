		...
		if (label.equalsIgnoreCase("setstat")) {
			if (!(args.length == 3)) {
				...
				try {
					...
					plugin.data.getConfig().set(target.getUniqueId().toString() + ".skyblock.stat." + stat.toString(),
							value);
					...
					plugin.utils.sendFormattedMessage(player, "&7Incorrect arguments. Use &a/liststats&7 for a list of stats or &a/help setstat&7.");
				}
